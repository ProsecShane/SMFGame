...
def initsprites():  # прогрузка спрайтов
	'''Initialize game's sprites based on mode'''
	global SP, SPRITES
...
def nroot(n, num):  # корень n-ой степени
	'''Get the nth root of given number'''
	return num ** (1 / float(n))


def interp(res):  # интерпретация строки к лямбда-функции
	'''Interpret a string, so that a lambda-function is returned'''
	try:
...
class Position:  # небольшой класс для удобного обращения через ".x" или ".y"
	'''A handy class for easier reference of x's and y's'''
	def __init__(self, x, y):
		self.x, self.y = x, y


class CoordsPlane:  # класс координатной плоскости
	'''Class that contains a coordinate plane'''
...
class Obstacle:  # класс-четырёхугольник со своими координатами, размером и спрайтом
	'''A base class for all rectangular objects'''
	...
	def point_in(self, x, y):  # находится ли точка в этом объекте
		'''Returns, whether the given point is within the obstacle'''
		return self.x <= x <= self.x + self.w and self.y <= y <= self.y + self.h

	def rect_in(self, x, y, width, height, recur=False):
		'''Returns, whether the given rectangle is within the obstacle'''
	...
	def draw(self):  # рисование объекта
		'''Draw the obstacle on the given surface with given coordinates, sizes and sprite'''
		self.sr.blit(self.sp, (int(self.x), int(self.y)))


class Button:  # класс-кнопка, основан на Obstacle
	'''A clickable rectangular button object, reacts to LMB clicks'''
	...
	def press(self):  # функция после нажатия кнопки
		'''Runs the procedure, which would run after pressing the button'''
		self.f(*self.a, **self.k)

	def react(self):  # выполнить функцию кнопки, если она нажата
		'''Buttons reaction, meaning it will run the given procedure if the button is pressed'''
		if self.is_pressed():
			self.press()
...
class Sprite:  # класс-спрайт
	'''Just a sprite object, doesn't require much info to create'''
...
class Menu:  # класс-меню, сборщик кнопок, спрайтов и др. объектов
	'''Menu object, contains multiple buttons'''
	...
	def show(self):  # показать, то есть при вызове рисования меню будет нарисовано
		'''Shows the menu, so that it will be drawn when the drawing is called'''
		self.shown = True

	def hide(self):  # скрыть, то есть при вызове рисования меню не будет нарисовано
		'''Hides the menu, so that it won't be drawn when the drawing is called'''
		self.shown = False

	def draw(self):  # нарисовать
		'''Draws all menu's objects, but only if it is shown'''
		if not self.shown:
	...
	def react(self):  # реакция меню
		'''Gets the reaction of all menu's objects, but only if it is shown'''
		if not self.shown:
...
class Level:  # класс-уровень, сборник препятствий, победы и спрайтов
	'''Level object, contains multiple obstacles, sprites and a goal'''
	def __init__(self, *objects):
		self.objects, self.goal, self.shown = list(objects), None, False

	def show(self):  # показать уровень
		'''Shows the level, so that it will be drawn when the drawing is called'''
		self.shown = True

	def hide(self):  # скрыть уровень
		'''Hides the level, so that it won't be drawn when the drawing is called'''
		self.shown = False

	def draw(self):
		'''Draws the all of the level's objects, but only if it is shown'''
		if not self.shown:
...
class Goal:  # класс-победа, основан на Obstacle
	'''Goal object, connects to a Level-object when created'''
...
class Player:  # класс-игрок, основан на Obstacle
	'''Player object, a moving Obstacle-object representing the player'''
	def __init__(self, x, y, width, height, sprite, surface, level):
		self.obst = Obstacle(x, y, width, height, sprite, surface)
	...
	def move(self, direc, amount):  # передвинуться в нужную сторону
		'''Moves the player for a needed amount in a needed direction'''
		if direc in ['u', 'd']:
	...
	def clip(self):  # застрял ли игрок в одном из объектов данного уровня
		'''Returns, where the player is clipping'''
	...
	def is_clipping(self):  # застревает хоть с какой-то стороны
		'''Returns, whether the player is clipping at all'''
		return any(list(self.clip().values()))

	def funcmove(self, func, afterfunc, *args, **kwargs):  # движение по функции
		'''Does nothing, searches for function, goes by a function, runc the win
		sequence or the lose sequence depending on player's state'''
		if self.state == 'none':
		...
		if self.state == 'func_search':  # при поиске функции
			...
			for i in range(int(self.obst.x), int(self.obst.x) + int(self.obst.w) + 1):
				i = (int(self.obst.w) + 1) // 2 + int(self.obst.x) + k
				k = (-k - 1) if k >= 0 else -k
...
class Game:  # класс-игра
	...
	def toLvlSelect(self, _):  # в "Выбор уровня"
		'''Changes screen to the level select screen'''
	...
	def clipped(self):  # игрок застрял
		'''The procedures for when the player gets stuck on some object'''
	...
	def toLevel(self, number):  # в уровень
		'''Changes screen to the needed level'''
	...
	def showlevel(self, number):  # показать уровень
		'''Shows only the given level, all others are hidden'''
	...
	def showmenu(self, menu, music, type):  # показать меню,
		'''Shows only the given menu, all others are hidden'''
	...
	def player_move(self, _):  # начать поиск функции у игрока
		'''Allows the player to search, whether it is on the function'''
		if self.player.state == 'none':
			self.player.state = 'func_search'

	def setup(self):  # перед главным циклом
		'''Steps before the main loop'''
		...
		objects = ([Obstacle(0, 26 * i, 26, 26, SP['lvlBlock'][2], self.main) for i in range(21)] +
				   [Obstacle(SIZE - 26, 26 * i, 26, 26, SP['lvlBlock'][2], self.main) 
				    for i in range(21)] +
				   [Obstacle(SIZE - 52, 26 * i, 26, 26, SP['lvlBlock'][2], self.main) 
				    for i in range(4, 21)] +
				   [Obstacle(SIZE - 78, 26 * i, 26, 26, SP['lvlBlock'][2], self.main) 
				    for i in range(4, 21)] +
		...
		self.level2goal = Goal(SIZE - 76, 54, 50, 50, SP['goal'], self.main, self.level2)
		...
		objects = ([Obstacle(0, 26 * i, 26, 26, SP['lvlBlock'][3], self.main) for i in range(21)] +
				   [Obstacle(SIZE - 26, 26 * i, 26, 26, SP['lvlBlock'][3], self.main) 
				    for i in range(21)] +
				   [Obstacle(SIZE // 2 - 26, 26 * i, 26, 26, SP['lvlBlock'][3], self.main)
				    for i in range(1, 7)] +
				   [Obstacle(SIZE // 2, 26 * i, 26, 26, SP['lvlBlock'][3], self.main)
				    for i in range(1, 7)] +
		...
		self.level3goal = Goal(SIZE - 76, 26, 50, 50, SP['goal'], self.main, self.level3)
		...
		objects = ([Obstacle(0, 26 * i, 26, 26, SP['lvlBlock'][4], self.main) for i in range(21)] +
				   [Obstacle(SIZE - 26, 26 * i, 26, 26, SP['lvlBlock'][4], self.main) 
				    for i in range(21)] +
				   [Obstacle(SIZE // 2, 26 * i, 26, 26, SP['lvlBlock'][4], self.main) 
				    for i in range(6, 20)] +
				   [Obstacle(SIZE // 2 + 26, 26 * i, 26, 26, SP['lvlBlock'][4], self.main) 
				    for i in range(6, 20)] +
		...
		self.level4goal = Goal(SIZE - 76, SIZE - 176, 50, 50, SP['goal'], self.main, self.level4)
		...
		objects = ([Obstacle(0, 26 * i, 26, 26, SP['lvlBlock'][5], self.main) for i in range(21)] +
				   ...
				   [Obstacle(26 * i, (39 / 2 * i + 150) - (39 / 2 * i + 150) % 26, 26, 26, 
				   	         SP['lvlBlock'][5], self.main) for i in range(1, 19)] +
				   [Obstacle(26 * i, (39 / 2 * i - 50) - (39 / 2 * i - 50) % 26, 26, 26, 
				   	         SP['lvlBlock'][5], self.main) for i in range(1, 24)])
		self.level5 = Level(*objects[:])
		self.level5goal = Goal(SIZE - 76, SIZE - 176, 50, 50, SP['goal'], self.main, self.level5)
		...
		objects = ([Obstacle(0, 26 * i, 26, 26, SP['lvlBlock'][6], self.main) for i in range(21)] +
				   [Obstacle(SIZE - 26, 26 * i, 26, 26, SP['lvlBlock'][6], self.main) 
				    for i in range(21)] +
				   [Obstacle(26 * 6, 26 * i, 26, 26, SP['lvlBlock'][6], self.main) 
				    for i in range(14)] +
				   [Obstacle(26 * i, 26 * 13, 26, 26, SP['lvlBlock'][6], self.main) 
				    for i in range(6, 25)] +
		...
		self.level6goal = Goal(SIZE - 76, SIZE - 230, 50, 50, SP['goal'], self.main, self.level6)
		...
		objects = ([Obstacle(0, 26 * i, 26, 26, SP['lvlBlock'][7], self.main) for i in range(21)] +
				   [Obstacle(SIZE - 26, 26 * i, 26, 26, SP['lvlBlock'][7], self.main) 
				    for i in range(21)] +
				   [Obstacle(75, 350, 26, 26, SP['lvlBlock'][7], self.main),
				    Obstacle(225, 150, 26, 26, SP['lvlBlock'][7], self.main),
				    Obstacle(400, 350, 26, 26, SP['lvlBlock'][7], self.main),
				    Obstacle(550, 150, 26, 26, SP['lvlBlock'][7], self.main)] +
		...
		self.level7goal = Goal(SIZE - 76, SIZE - 176, 50, 50, SP['goal'], self.main, self.level7)
		...
		objects = ([Obstacle(0, 26 * i, 26, 26, SP['lvlBlock'][8], self.main) for i in range(21)] +
				   [Obstacle(SIZE - 26, 26 * i, 26, 26, SP['lvlBlock'][8], self.main) 
				    for i in range(21)] +
				   [Obstacle(26 * i, 26 * 5, 26, 26, SP['lvlBlock'][8], self.main) 
				    for i in range(25)] +
				   [Obstacle(26 * i, 26 * 16, 26, 26, SP['lvlBlock'][8], self.main) 
				    for i in range(9, 25)] +
				   [Obstacle(26 * 9, 26 * i, 26, 26, SP['lvlBlock'][8], self.main) 
		...
		self.level8goal = Goal(SIZE - 76, 26 * 7, 50, 50, SP['goal'], self.main, self.level8)
		...
		objects = ([Obstacle(0, 26 * i, 26, 26, SP['lvlBlock'][9], self.main) for i in range(21)] +
				   [Obstacle(SIZE - 26, 26 * i, 26, 26, SP['lvlBlock'][9], self.main) 
				    for i in range(21)] +
				   [Obstacle(26 * 6, 26 * i, 26, 26, SP['lvlBlock'][9], self.main) 
				    for i in range(13)] +
				   [Obstacle(SIZE - 26 * 7, 26 * i, 26, 26, SP['lvlBlock'][9], self.main) 
				    for i in range(13)] +
				   [Obstacle(26 * i, 26 * 13, 26, 26, SP['lvlBlock'][9], self.main) 
				    for i in range(6, 19)] +
		...
		self.level9goal = Goal(SIZE - 76, 26, 50, 50, SP['goal'], self.main, self.level9)
		...
		objects = ([Obstacle(0, 26 * i, 26, 26, SP['lvlBlock'][10], self.main) for i in range(21)] +
				   [Obstacle(SIZE - 26, 26 * i, 26, 26, SP['lvlBlock'][10], self.main) 
				    for i in range(21)] +
				   [Obstacle(26 * 8, 26 * i, 26, 26, SP['lvlBlock'][10], self.main) 
				    for i in range(5)] +
				   [Obstacle(26 * 17, 26 * i, 26, 26, SP['lvlBlock'][10], self.main) 
				    for i in range(16, 21)] +
				   [Obstacle(26 * i, 26 * 16, 26, 26, SP['lvlBlock'][10], self.main) 
				    for i in range(17)] + 
				   [Obstacle(26 * i, 26 * 4, 26, 26, SP['lvlBlock'][10], self.main) 
				    for i in range(8, 25)] + 
		...
		self.level10goal = Goal(SIZE - 76, SIZE - 176, 50, 50, SP['goal'], self.main, self.level10)
		...
		self.defCoords = {1: (30, SIZE - 230), 2: (30, SIZE - 430), 3: (30, 30),
		                  4: (30, SIZE - 230), 5: (30, 30), 6: (30, 30),
		                  7: (50, 30), 8: (30, SIZE - 230), 9: (30, 30),
		                  10: (30, 30), 0: (-10000, -10000)}
	...
	def set_flag(self, key, value):
		'''Sets a certain flag a certain value'''
	...
	def mainloop(self):  # главный цикл
		...
		while self.run:
			...
			if self.flags['in_lvl'] and self.player.state == 'none':
